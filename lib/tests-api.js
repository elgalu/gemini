...
function SuiteBuilder(suite) {
    ...
    this.setRetries = function(retries) {
        if (typeof retries !== 'number') {
            throw new TypeError('retries must be a number');
        }

        if (retries !== parseInt(retries, 10)) {
            throw new TypeError('retries must be integer number');
        }
